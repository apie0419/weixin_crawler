...
#     results.to_csv("week_count.csv", encoding = "utf-8")
...
def get_week_countrate_plot(df, start, end):
    ...
    while True:
        ...
        total = all_df.size
        official = all_df.loc[df["official"] == 1].size
        forprofit = all_df.loc[df["forprofit"] == 1].size
        license = all_df.loc[df["license"] == 1].size
